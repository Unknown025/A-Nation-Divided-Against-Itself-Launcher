...
class Profile:
    def __init__(self, version):
        ...
        try:
            ...
            mkdir("mcdata/versions/%s" % self.version)
            self.cdownload1 = threading.Thread(target=download_file,
                                               args=("mcdata/versions/%s/%s.json" % (self.version, self.version),
                                                     "http://s3.amazonaws.com/Minecraft.Download/versions/%s/%s.json" % (
                                                     self.version, self.version))
                                               )
                ...
                if currentOS in libinfo["natives"]:
                    ...
                    if not os.path.exists(libpath):
                        ...
                        try:
                            ...
                            logging.exception(
                                "Zip/Jar file could not be extracted: %s. Error goes as follows: %s" % (name, e))
                            print("[%s WARN]: A faulty zip or jar file was found. This may cause problems!" % (
                                time.strftime("%H:%M:%S")))
                            try:
                                os.remove(libpath)
                            except:
                            ...
                            "[%s WARN]: Jar file or zip file not found. This may cause errors." % (
                                time.strftime("%H:%M:%S"))

        # We must also get the assets index.
        assets_name = self.versionInfo.get("assets", "legacy")
        assets_index_file = "mcdata/assets/indexes/%s.json" % assets_name
        assets_index_link = "https://s3.amazonaws.com/Minecraft.Download/indexes/%s.json" % assets_name
        self.download3 = threading.Thread(target=download_file, args=(assets_index_file, assets_index_link))
        self.download3.start()
        self.download3.join()
        assets_index_file = str(assets_index_file)
        f = open(assets_index_file, "rb")
        assets_data = json.loads(f.read())
        f.close()
            ...
            if not os.path.exists(filename):
                if len(running_downloads) > 49:
                    for f in running_downloads:
                        f.join()
                    running_downloads = []
                downloadmfiles = threading.Thread(target=download_file, args=(filename, url))
    ...
    def download_forge():
        ...
        urlretrieve(
            "http://anationdividedagainstitself-modpack.rhcloud.com/a-nation-divided-against-itself.json",
            "a-nation-divided-against-itself.json")
        urlretrieve(
            "http://files.minecraftforge.net/maven/net/minecraftforge/forge/1.7.10-10.13.4.1614-1.7.10/forge-1.7.10"
            "-10.13.4.1614-1.7.10-universal.jar",
            "forge-universal.jar")
        try:
        ...
        for i in dn["tasks"]:
            if not os.path.isfile(dn["tasks"][x]["to"]):
                ...
                download_forge = threading.Thread(target=download_file, args=(filename, url))
                download_forge.start()
                running_downloads.append(download_forge)
    ...
    def launch_cmd(self, username="MinecraftPlayer"):
        libs = [self.jar]
